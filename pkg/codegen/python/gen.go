	...
	genTypes := func(file string, input bool) error {
		...
		addImport := func(tok string, input bool) {
			if imp := mod.importFromToken(tok, input); imp != "" {
				if (input && imp == "from ._inputs import *") || (!input && imp == "from . import outputs") {
					return
				}
				imports.add(imp)
			}
...
func (mod *modContext) genAwaitableType(w io.Writer, obj *schema.ObjectType) string {
	...
	fmt.Fprintf(w, "    # pylint: disable=no-self-argument\n")
...
func (mod *modContext) genResource(res *schema.Resource) (string, error) {
	...
	fmt.Fprintf(w, "    # pylint: disable=no-self-argument\n")
	fmt.Fprintf(w, "    def __init__(__self__, resource_name, opts: Optional[pulumi.ResourceOptions] = None")
	...
	fmt.Fprintf(w, ", __props__=None, __name__=None, __opts__=None) -> None:\n")
...
func (mod *modContext) genFunction(fun *schema.Function) (string, error) {
	...
	if len(args) > 0 {
		fmt.Fprintln(docs, "")
		for _, arg := range args {
			mod.genPropDocstring(docs, arg, false /*wrapInput*/, true /*orDict*/)
		}
	}
...
func (mod *modContext) genInitDocstring(w io.Writer, res *schema.Resource) {
	...
	for _, prop := range res.InputProperties {
		mod.genPropDocstring(b, prop, true /*wrapInput*/, true /*orDict*/)
	}
...
func (mod *modContext) genTypeDocstring(w io.Writer, comment string, properties []*schema.Property, wrapInput bool) {
	...
	for _, prop := range properties {
		mod.genPropDocstring(b, prop, wrapInput, false /*orDict*/)
	}
...
func (mod *modContext) genPropDocstring(w io.Writer, prop *schema.Property, wrapInput, orDict bool) {
	if prop.Comment == "" {
		return
	}
...
func (mod *modContext) genType(
	...
	if input {
		...
		for _, prop := range props {
			...
			fmt.Fprintf(w, ", %s: %s%s", pname, ty, defaultValue)
		}
		fmt.Fprintf(w, ") -> None:\n")
...
func generateModuleContextMap(tool string, pkg *schema.Package, info PackageInfo, extraFiles map[string][]byte) (map[string]*modContext, error) {
	...
	for _, v := range pkg.Config {
		visitObjectTypes(v.Type, func(t *schema.ObjectType) {
